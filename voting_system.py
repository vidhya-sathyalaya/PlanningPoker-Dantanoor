...
def pollpage(): 
     ...
     ppage.title('Poll')
     Label(ppage,text='Vote').grid(row=1,column=3)
...
def create():
    def proceed():
        ...
        can=cname.get()   
        if pname=='':
            return messagebox.showerror('Error','Enter exercise name')
        elif can=='':
            return messagebox.showerror('Error','Enter votes')
        else:
    ...
    cr=Toplevel()
    cr.geometry('500x400')
    cr.title('Create a new exercise')
    Label(cr,text='Enter Details',font='Helvetica 12 bold').grid(row=1,column=2)
    Label(cr,text='Enter Exercise name: ').grid(row=2,column=1)
    Entry(cr,width=30,textvariable=name).grid(row=2,column=2) #poll name
    Label(cr,text='(eg: Exercise 1)').place(x=354,y=25)
    Label(cr,text='Enter Points: ').grid(row=3,column=1)
    Entry(cr,width=45,textvariable=cname).grid(row=3,column=2) #candidate name
    Label(cr,text="Note: Enter the points for the user story by ','").grid(row=4,column=2)
    Label(cr,text='eg: 1,2,3,4,5....').grid(row=5,column=2)
...
def about():
    messagebox.showinfo('About','Developed by Kaushik Reddy ASU id - 1224356637')
...
home['bg'] = '#fff'
Label(home,text='Planning Poker Scrum Exercise',font='Helvetica 12 bold').grid(row=1,column=3)
Button(home,text='Create new Poll +',command=create).grid(row=5,column=3)
Button(home,text='My Polls',command=polls).grid(row=6,column=3)
Button(home,text='Poll Results',command=selpl).grid(row=7,column=3)
Button(home,text='About',command=about).grid(row=8,column=3)
home.mainloop()
